...
export default function EditchartOfAccounts({ closeModal }: CreateModalProps) {
    ...
    const id_do_plano = cookie.ID_PLANO
    ...
    const registerChart = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        let verb
          ...
          url = `http://localhost:3333/api/plano_contas/${id_do_plano}`
          verb = 'PUT'
    ...
    const editar = useEffect(() => {
        ...
        if(Number(id_do_plano) != 0)
        {
            if(id_do_plano != undefined){
                const findobj = async () => {
                    try {
                        const answer = await fetch(`http://localhost:3333/api/plano_contas/${id_do_plano}`)
                        const obj = await answer.json();
                        if(answer.ok)
                        {   
                            setID(Number(obj.ID))              
                            setCod1(Number(obj.COD_NIVE1))  
                            setDesc1(obj.DESC_NIVEL1);
                            setCod2(Number(obj.COD_NIVE2));
                            setDesc2(obj.DESC_NIVEL2);
                            setCod3(Number(obj.COD_NIVE3));
                            setDesc3(obj.DESC_NIVEL3);
