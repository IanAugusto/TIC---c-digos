...
export default function EditProduct({ closeModal }: CreateModalProps) {
...
/*fim get unidade de medida */
    ...
    const id_do_produto = cookie.ID_PRODUTO


    const [products, setProducts] = useState<DeclareProduct[]>([])

    const [ID, setID] = useState(Number(id_do_produto))
    ...
    const registerProduct = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        let verb
          ...
          url = `http://localhost:3333/api/produto/${id_do_produto}`
          verb = 'PUT'
    ...
    /**PEGA O PRODUTO ATRAVÉS DO COOKIE */
    const editar = useEffect(() => {

        if(Number(id_do_produto) != 0)
        {
            if(id_do_produto != undefined){
                const findobj = async () => {
                    try {
                        const answer = await fetch(`http://localhost:3333/api/produto/${id_do_produto}`)
                        const obj = await answer.json();
                        if(answer.ok)
                        {               
                            setID(Number(obj.ID))  
                            setCodMa(obj.COD_MATERIAL);
                            setDesc(obj.DESCRICAO);
                            setMarca(obj.MARCA);
                            setTipo(obj.TIPO_PROD_ID);
                            setSta(obj.STA_ATIVO);
                            setMin(Number(obj.QTD_ESTOQUE_MINIMO));
                            setMax(Number(obj.QTD_ESTOQUE_MAXIMO));                                                                            
                            setUnme(obj.ID_MEDIDA);
