...
export default function EditUser({ closeModal }: CreateModalProps) {
    ...
    const id_do_usuario = cookie.ID_USUARIO
    ...
    const registerUser = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        url = `http://localhost:3333/api/usuario/${id_do_usuario}`
        verb = 'PUT'
    ...
    const editar = useEffect(() => {
        ...
        if(Number(id_do_usuario) != 0)
        {
            if(id_do_usuario != undefined){
                const findobj = async () => {
                    try {
                        const answer = await fetch(`http://localhost:3333/api/usuario/${id_do_usuario}`)
    ...
    return (
        <form className="space-y-8 divide-y divide-gray-200" onSubmit={registerUser}>
            ...
            <div className="space-y-8 divide-y divide-gray-200">
                <div>
                    <div>
                        <h3 className="text-lg leading-6 font-medium text-gray-900">Editar um Usuário</h3>
