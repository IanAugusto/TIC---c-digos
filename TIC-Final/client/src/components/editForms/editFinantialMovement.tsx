...
export default function EditFinantialMovement({ closeModal }: CreateModalProps) {
    ...
    const id_da_mov = cookie.ID_FIN_MOV
    ...
    const registerMove = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        let verb
          ...
          url = `http://localhost:3333/api/movimentacao_financeira/${id_da_mov}`
          verb = 'PUT'
    ...
    const editar = useEffect(() => {
        ...
        if(Number(id_da_mov) != 0)
        {
            if(id_da_mov != undefined){
                const findobj = async () => {
                    try {
                        const answer = await fetch(`http://localhost:3333/api/movimentacao_financeira/${id_da_mov}`)
                        const obj = await answer.json();
                        if(answer.ok)
                        {               
                            setID(Number(obj.ID))  
                            setES(obj.E_S);
                            setCentro(obj.ID_CENTRO_CUSTO);
                            setDataMov(obj.DATA_MOVTO);
                            setOrigem(obj.ID_ORIG_RECEITA);
                            setValor(obj.VALOR);
                            setForn(obj.ID_FORN);
                            setPlano(obj.ID_PLANO);                                                                            
...
function eliminaCookie() {
    // remove o cookie ID_FIN_MOV
    removeCookie('ID_FIN_MOV')
}
