...
export default function EditSupplier({ closeModal }: CreateModalProps) {
    ...
    const id_do_fornecededor = cookie.ID_FORN
    ...
    const registerSupplier = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        let verb
          ...
          url = `http://localhost:3333/api/fornecedor/${id_do_fornecededor}`
          verb = 'PUT'
     ...
     /*fim da função para criar o fornecedor */
       ...
       /**PEGA O FORNECEDOR ATRAVÉS DO COOKIE */
    const editar = useEffect(() => {

        if(Number(id_do_fornecededor) != 0)
        {
            if(id_do_fornecededor != undefined){
                const findobj = async () => {
                    try {
                        const answer = await fetch(`http://localhost:3333/api/fornecedor/${id_do_fornecededor}`)
                        const obj = await answer.json();
                        if(answer.ok)
                        {               
                            setID(Number(obj.ID))  
                            setPessoa(obj.PESSOA);
                            setNomPessoa(obj.NOM_PESSOA);
                            setCNPJ(obj.NUM_CGC);
                            setCpf(obj.NUM_CPF);
                            setEmail(obj.EMAIL);
                            setContato(obj.NOM_CONTATO);
                            setStaAtivo(obj.STA_ATIVO);                                                                            
                            setNomeApelido(obj.NOM_APELIDO);
                            setNomeFantasia(obj.NOM_FANTASIA);
                            setDesHistorico(obj.DES_HISTORICO);
                            setNomeCargo(obj.NOM_CARGO);
                            setRamoAtiv(obj.RAMO_ATIVIDADE);
                            setFone(obj.FONE);
                            setNfe(obj.NFE);
                            setWeb(obj.WEBSITE);
                            setNumInsc(obj.NUM_INSC_ESTATUAL);
...
function eliminaCookie() {
    // remove o cookie ID_FORN
    removeCookie('ID_FORN')
}
