...
export default function EditSupplierProduct({ closeModal }: CreateModalProps) {
    ...
    const id_forn_prod = cookie.ID_FORN_PROD
    //
    ...
    const registerAssociation = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        let url 
        let verb 
            url = `http://localhost:3333/api/fornecedor_produto/${id_forn_prod}`
            verb = 'PUT'
...
/**Fim get fornecedores */
    ...
    /**PEGA A ASSOCIAÇÃO ATRAVÉS DO COOKIE */
    const editar = useEffect(() => {

        if(Number(id_forn_prod) != 0)
        {
            if(id_forn_prod != undefined){
                const findobj = async () => {
                    try {
                        const answer = await fetch(`http://localhost:3333/api/fornecedor_produto/${id_forn_prod}`)
